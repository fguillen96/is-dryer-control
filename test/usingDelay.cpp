...
const int kNumberModulesSeries = 17;            // Number of modules in series
const int kVmppModule = 1;                      // Maximum power point voltage of module
const int kVmppMax = 1;                         // Maximun power point voltage
const int kVmppMin = 1;                         // Minium power point voltage
const int kDVocDt = 1;                          // Voc variation with temperature (V/ºC)
const int kGMinToOff = 400;                     // Minimum value of irradiance to stop drive
const int kGThldToOn = 800;                     // Irradiance value trehsold to start drive
const int kVdcMinMot = 1;                       // Minimum DC voltage with maximum power pump

const float kFmaxOut = 1;                       // Max output frequency
...
void loop() {
  ...
  if (SecondsOverIrradianceThld(irradiance, kGThldToOn) > 60) run_allowed = true;

  bool stop_needed = false;
  if (SecondsUnderIrradianceThld(irradiance, kGMinToOff) > 60) stop_needed = true;
  ...
  switch (state) {
    ...
    case stopped: {
      ...
      if (run_signal_on && run_allowed) {
        state = OnMptt;
        digitalWrite(DO_RAMP_CHANGE, LOW);
        delay(200);
        //TODO: HIGH OR LOW?
      }
      if (alarm) state = Alarm;
    ...
    case OnMptt: {
      ...
      if (!run_signal_on || stop_needed) {
        state = stopping;
        digitalWrite(DO_RAMP_CHANGE, HIGH);
        delay(200);
        //TODO: HIGH OR LOW???
      }
...
int SecondsOverIrradianceThld(int irradiance, int threshold) {
...
int SecondsUnderIrradianceThld(int irradiance, int threshold) {
